...
// Event listener - grabs name of location and address from user input. Changed "submit" id to "autoComplete"
    ...
    function queryPlace() {
        ...
        service.nearbySearch(request, (results, status) => {
            if (status === google.maps.places.PlacesServiceStatus.OK) {
                ...
                console.log("lat ", lat);
                console.log("lng ", lng);
                let firstResults = results[0];

                service.getDetails({placeId: results[0].place_id}, (results, status)=> {
                    console.log(results);
                    if (status === google.maps.places.PlacesServiceStatus.OK) {
